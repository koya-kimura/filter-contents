/* **************************************************************************** */
/* <注意>                                                                       */
/* - 本ソースファイルはクラスコンポーネントでp5.jsのスケッチを実装したものである    */
/* - これを関数コンポーネントにリファクタリングしたものがCameraCapture.jsである     */
/* - state型のshaderIndex変数を同期するにあたり，本ソースコードの利用を廃止している */
/* **************************************************************************** */
...
export default class CameraCapture extends Component {
  constructor(props) {
    super(props);
    this.shaderIndex = props.shaderIndex;
  ...
  sketchHandler = p => {
    ...
    p.draw = () => {
      ...
      filterShaders[this.shaderIndex].setUniform("u_tex", capture);
